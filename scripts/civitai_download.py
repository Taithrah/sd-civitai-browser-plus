...
try:
    ...
    print(
        "Python module 'ZipUnicode' has not been imported correctly, please try to restart or install it manually."
    )
...
if os_type == "Windows":
    ...
    aria2 = os.path.join(aria2path, "linux", "aria2")
    st = os.stat(aria2)
...
def create_model_item(
    ...
    (preview_html, _, _, _, _, _, _, _, _, _, _, existing_path, _) = (
        _api.update_model_info(None, model_versions.get("value"), False, model_id)
    )
...
def selected_to_queue(model_list, subfolder, download_start, create_json, current_html):
    ...
    for model_string in model_list:
        ...
        for item in gl.json_data["items"]:
            if int(item["id"]) == int(model_id):
                ...
                primary_file = next(
                    (file for file in files if file.get("primary", False)), None
                )
                if primary_file:
    ...
    return (
        ...
        gr.Button.update(
            interactive=True if len(gl.download_queue) > 1 else False, visible=True
        ),  # Cancel All Button
...
def download_finish(model_filename, version, model_id):
    ...
    return (
        gr.Button.update(
            interactive=model_filename, visible=Down, value="Download model"
        ),  # Download Button
...
def get_download_link(url, model_id):
    ...
    response = requests.get(
        url, headers=headers, allow_redirects=False, proxies=proxies, verify=ssl
    )

    if 300 <= response.status_code <= 308:
        if (
            "login?returnUrl" in response.text
            and "reason=download-auth" in response.text
        ):
            return "NO_API"
...
def download_file(
    url, file_path, install_path, model_id, progress=gr.Progress() if queue else None
):
    try:
        ...
        if not download_link:
            print(
                f'File: "{file_name}" not found on CivitAI servers, it looks like the file is not available for download.'
            )
            gl.download_fail = True
            return
            ...
            print(
                f'File: "{file_name}" requires a personal CivitAI API to be downloaded, you can set your own API key in the CivitAI Browser+ settings in the SD-WebUI settings tab'
            )
            gl.download_fail = "NO_API"
            if progress != None:
                progress(
                    0,
                    desc=f'File: "{file_name}" requires a personal CivitAI API to be downloaded, you can set your own API key in the CivitAI Browser+ settings in the SD-WebUI settings tab',
                )
                time.sleep(5)
            return
            ...
            try:
                ...
                if progress != None:
                    progress(
                        ...
                        desc=f"Downloading: {file_name} - {convert_size(completed_length)}/{convert_size(total_length)} - Speed: {convert_size(download_speed)}/s - ETA: {eta_formatted} - Queue: {current_count}/{total_count}",
                    )
                ...
                if status_info["status"] == "error":
                    if progress != None:
                        progress(
                            0,
                            desc=f'Encountered an error during download of: "{file_name}" Please try again.',
                        )
                    gl.download_fail = True
                    return
...
def download_file_old(
    url, file_path, model_id, progress=gr.Progress() if queue else None
):
    try:
        gl.download_fail = False
        max_retries = 5
        ...
        if not download_link:
            print(
                f'File: "{file_name_display}" not found on CivitAI servers, it looks like the file is not available for download.'
            )
            if progress != None:
                progress(
                    0,
                    desc=f'File: "{file_name_display}" not found on CivitAI servers, it looks like the file is not available for download.',
                )
                time.sleep(5)
            gl.download_fail = True
            return
            ...
            print(
                f'File: "{file_name_display}" requires a personal CivitAI API key to be downloaded, you can set your own API key in the CivitAI Browser+ settings in the SD-WebUI settings tab'
            )
            gl.download_fail = "NO_API"
            if progress != None:
                progress(
                    0,
                    desc=f'File: "{file_name_display}" requires a personal CivitAI API key to be downloaded, you can set your own API key in the CivitAI Browser+ settings in the SD-WebUI settings tab',
                )
                time.sleep(5)
            return
                    ...
                    try:
                        ...
                        try:
                            ...
                            if response.status_code == 404:
                                if progress != None:
                                    progress(
                                        0,
                                        desc=f"Encountered an error during download of: {file_name_display}, file is not found on CivitAI servers.",
                                    )
                                gl.download_fail = True
                                return
                                ...
                                if current_time - last_update_time >= update_interval:
                                    if progress != None:
                                        progress(
                                            ...
                                            desc=f"Downloading: {file_name_display} {convert_size(downloaded_size)} / {convert_size(total_size)} - Speed: {convert_size(int(download_speed))}/s - ETA: {eta_formatted} - Queue: {current_count}/{total_count}",
                                        )
                        ...
                        if max_retries == 0:
                            if progress != None:
                                progress(
                                    0,
                                    desc=f"Encountered an error during download of: {file_name_display}, please try again.",
                                )
                            gl.download_fail = True
                            return
                        time.sleep(5)
...
def download_create_thread(
    download_finish, queue_trigger, progress=gr_progress_threadable() if queue else None
):
    ...
    if not gl.download_queue:
        return (
            ...
            gr.Textbox.update(
                value=random_number(download_finish)
            ),  # Download Finish Trigger
    ...
    if use_aria2 and os_type != "Darwin":
        ...
        thread = threading.Thread(
            target=download_file_old,
            args=(item["dl_url"], path_to_new_file, item["model_id"], progress),
        )
    thread.start()
            ...
            if unpack_zip:
                try:
                    if path_to_new_file.endswith(".zip"):
                        ...
                        print(
                            f"Successfully extracted {item['model_filename']} to {directory}"
                        )
                        os.remove(path_to_new_file)
                except ImportError:
                    print(
                        "Python module 'ZipUnicode' has not been imported correctly, cannot extract zip file. Please try to restart or install it manually."
                    )
                except Exception as e:
                    print(f"Failed to extract {item['model_filename']} with error: {e}")
            if not gl.cancel_status:
                if item["create_json"]:
                    _file.save_model_info(
                        item["install_path"],
                        item["model_filename"],
                        item["sub_folder"],
                        item["model_sha256"],
                        item["preview_html"],
                        api_response=item["model_json"],
                    )
                info_to_json(
                    path_to_new_file, item["model_id"], item["model_sha256"], unpackList
                )
                _file.save_preview(
                    path_to_new_file, item["model_json"], True, item["model_sha256"]
                )
                if save_all_images:
                    _file.save_images(
                        item["preview_html"],
    ...
    if gl.cancel_status:
        ...
        model_string = f"{item['model_name']} ({item['model_id']})"
        (card_name, _, _) = _file.card_update(
            item["model_versions"], model_string, item["version_name"], True
        )
    ...
    return (
        ...
        gr.Button.update(
            interactive=True if len(gl.download_queue) > 1 else False
        ),  # Cancel All Button
    )
...
def get_style(size, left_border):
    return (
        f"flex-grow: {size};"
        + ("border-left: 1px solid var(--border-color-primary);" if left_border else "")
        + "padding: 5px 10px 5px 10px;width: 0;align-self: center;"
    )
