...
def contenttype_folder(content_type, desc=None, fromCheck=False, custom_folder=None):
    ...
    if content_type == "Checkpoint":
        ...
        if (
            hasattr(cmd_opts, "controlnet_dir")
            and cmd_opts.controlnet_dir
...
def model_list_html(json_data):
            ...
            if hide_early_access:
                ...
                if early_access_deadline_str:
                    early_access_deadline = datetime.strptime(
                        early_access_deadline_str, "%Y-%m-%dT%H:%M:%S.%fZ"
    ...
    for item in json_data["items"]:
        model_folder = os.path.join(
            contenttype_folder(item["type"], item["description"])
        )
                    ...
                    with open(json_path, "r", encoding="utf-8") as f:
                        try:
                            ...
                            if isinstance(json_file, dict):
                                ...
                                print(
                                    f"Invalid JSON data in {json_path}. Expected a dictionary."
                                )
    ...
    for item in json_data["items"]:
        ...
        if any(item["modelVersions"]):
            ...
            if installstatus != "civmodelcardinstalled":
                model_card += (
                    f'<input type="checkbox" class="model-checkbox" id="checkbox-{model_string}" onchange="multi_model_select(\'{model_string}\', \'{item["type"]}\', this.checked)" style="opacity: 0; position: absolute; top: 10px; right: 10px;">'
...
def create_api_url(
    ...
    params = {
        "limit": tile_count,
        "sort": sort_type,
        "period": period_type.replace(" ", "") if period_type else None,
    }
    ...
    query_string = urllib.parse.urlencode(
        query_parts, doseq=True, quote_via=urllib.parse.quote
    )
...
def initial_model_page(
    ...
    return (
        gr.Dropdown.update(
            choices=model_list, value="", interactive=True
        ),  # Model List
        ...
        gr.Button.update(
            interactive=False, visible=False if gl.isDownloading else True
        ),  # Download Button
    ...
    for item in api_json["items"]:
        if int(item["id"]) == int(model_id):
            ...
            display_version_names = [
                f"{v} [Installed]" if v in installed_versions else v
                for v in version_names
            ]
            default_installed = next(
                (f"{v} [Installed]" for v in installed_versions), None
            )
            default_value = default_installed or next(iter(version_names), None)

            return gr.Dropdown.update(
                choices=display_version_names, value=default_value, interactive=True
            )  # Version List
    ...
    if model_id:
        ...
        for item in api_data["items"]:
            if int(item["id"]) == int(model_id):
                ...
                uploader_avatar = (
                    f'<div class="avatar"><img src={uploader_avatar}></div>'
                )
                tags = item.get("tags", "")
                model_desc = item.get("description", "")
                if model_desc:
                    model_desc = model_desc.replace(
                        "<img", '<img style="max-width: -webkit-fill-available;"'
                    )
                    model_desc = model_desc.replace(
                        "<code>", '<code style="text-wrap: wrap">'
                    )
                ...
                try:
                    model_date_published = selected_version.get(
                        "publishedAt", ""
                    ).split("T")[0]
                except:
                ...
                for file in selected_version["files"]:
                    ...
                    if not model_filename:
                        ...
                        model_filename = (
                            f"{model_filename}_{file['id']}{model_extension}"
                        )
                    ...
                    file_dict.append({"format": format, "sizeKB": sizeKB})
                    if is_primary:
                    ...
                    if safe_tensor_found and pickle_tensor_found:
                        if "PickleTensor" in file_dict[0].get("format", ""):
                            if file_dict[0].get("sizeKB", 0) <= 100:
                                model_folder = os.path.join(
                                    contenttype_folder("TextualInversion")
                                )
                ...
                for index, pic in enumerate(api_version["images"]):
                    ...
                    image_url = re.sub(
                        r"/width=\d+", f"/width={pic['width']}", pic["url"]
                    )
                    if pic["type"] == "video":
                    ...
                    if prompt_dict:
                        ...
                        remaining_keys = [
                            key for key in prompt_dict if key not in preferred_order
                        ]
                ...
                tags_html = "".join(
                    [
                        f'<span class="civitai-tag">{escape(str(tag))}</span>'
                        for tag in tags
                    ]
                )
                ...
                version_url = (
                    f"{model_main_url}?modelVersionId={selected_version['id']}"
                )
        ...
        for root, dirs, files in os.walk(model_folder, followlinks=True):
            for filename in files:
                ...
                for filename in files:
                    if (
                        filename.lower() == model_filename.lower()
                        or filename.lower() == cleaned_name(model_filename).lower()
                    ):
                        folder_location = root
                        BtnDownInt = False
                        BtnDel = True
        ...
        if len(gl.download_queue) > 0:
            ...
            for item in gl.download_queue:
                if item["version_name"] == model_version and int(
                    item["model_id"]
                ) == int(model_id):
                    BtnDownInt = False
                    break
        ...
        return (
            ...
            gr.Button.update(
                visible=False if BtnDel else True,
                interactive=BtnDownInt,
                value=BtnDownTxt,
            ),  # Download Button
            gr.Button.update(interactive=BtnImage),  # Images Button
            gr.Button.update(visible=BtnDel, interactive=BtnDel),  # Delete Button
            gr.Dropdown.update(
                choices=file_list, value=default_file, interactive=True
            ),  # File List
            gr.Textbox.update(
                value=cleaned_name(model_filename), interactive=True
            ),  # Model File Name
            ...
            gr.Textbox.update(
                interactive=True, value=folder_path if model_name else None
            ),  # Install Path
            gr.Dropdown.update(
                choices=sub_folders, value=default_subfolder, interactive=True
            ),  # Sub Folder List
        )
    else:
        return (
            ...
            gr.Button.update(
                visible=False if BtnDel else True, value="Download model"
            ),  # Download Button
                ...
                for model in item["modelVersions"]:
                    if model["name"] == model_version:
                        ...
                        if is_LORA and file_list:
                            extracted_formats = [
                                file.split(" ")[1] for file in file_list
                            ]
                            if (
                                "SafeTensor" in extracted_formats
                                and "PickleTensor" in extracted_formats
                            ):
                                embed_check = True
                        ...
                        for file in model["files"]:
                            ...
                            if (
                                f"{file_size} {file_format} {file_fp} ({filesize})"
                                == file_metadata
                            ):
                                installed = False
                                folder_location = "None"
                                model_folder = os.path.join(
                                    contenttype_folder(content_type, desc)
                                )
                                        ...
                                        for filename in files:
                                            if filename.endswith(".json"):
                                                with open(
                                                    ...
                                                    try:
                                                        data = json.load(f)
                                                        sha256_value = data.get(
                                                            "sha256"
                                                        )
                                                        if (
                                                            sha256_value != None
                                                            and sha256_value.upper()
                                                            == sha256
                                                        ):
                                                            folder_location = root
                                                            installed = True
                                ...
                                return (
                                    gr.Textbox.update(
                                        value=cleaned_name(file["name"]),
                                        interactive=True,
                                    ),  # Model File Name Textbox
                                    gr.Textbox.update(
                                        value=dl_url
                                    ...
                                    gr.Button.update(
                                        interactive=BtnDownInt,
                                        visible=False if installed else True,
                                        value=BtnDownTxt,
                                    ),  # Download Button
                                    gr.Button.update(
                                        interactive=True if installed else False,
                                        visible=True if installed else False,
                                    ),  # Delete Button
                                    gr.Textbox.update(
                                        interactive=True,
                                        value=folder_path if model_name else None,
                                    ),  # Install Path
                                    gr.Dropdown.update(
                                        value=default_subfolder, interactive=True
                                    ),  # Sub Folder List
                                )
...
def request_civit_api(api_url=None, skip_error_check=False):
    ...
    try:
        response = requests.get(
            api_url, headers=headers, timeout=(60, 30), proxies=proxies, verify=ssl
        )
        if skip_error_check:
...
def api_error_msg(input_string):
    ...
    if input_string == "not_found":
        return (
            div
            + "Model ID not found on CivitAI.<br>Maybe the model doesn't exist on CivitAI?</div>"
        )
        ...
        return (
            div
            + "The CivitAI-API has timed out, please try again.<br>The servers might be too busy or down if the issue persists."
        )
    elif input_string == "offline":
        return (
            div
            + "The CivitAI servers are currently offline.<br>Please try again later."
        )
    elif input_string == "no_items":
        return (
            div
            + "Failed to retrieve any models from CivitAI<br>The servers might be too busy or down if the issue persists."
        )
    else:
        return (
            div
            + "The CivitAI-API failed to respond due to an error.<br>Check the logs for more details."
        )
