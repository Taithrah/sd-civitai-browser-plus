...
try:
    ...
    print(
        "Python module 'send2trash' has not been imported correctly, please try to restart or install it manually."
    )
try:
    from bs4 import BeautifulSoup
except ImportError:
    print(
        "Python module 'BeautifulSoup' has not been imported correctly, please try to restart or install it manually."
    )
...
def delete_model(
    ...
    (model_name, ver_value, ver_choices) = _file.card_update(
        model_versions, model_string, list_versions, False
    )
    if not model_json:
        if model_id != None:
                    ...
                    if file_sha256 == sha256_upper:
                        ...
                        for unpacked_file in unpack_list:
                            ...
                            if os.path.isfile(unpacked_file_path):
                                try:
                                    ...
                                    print(
                                        f"File moved to trash based on unpackList: {unpacked_file_path}"
                                    )
                                except:
                                    os.remove(unpacked_file_path)
                                    print(
                                        f"File deleted based on unpackList: {unpacked_file_path}"
                                    )
                        ...
                        if os.path.isfile(file_path):
                            try:
                                ...
                                print(
                                    f"Model moved to trash based on SHA-256: {file_path}"
                                )
                            except:
                                os.remove(file_path)
                ...
                if filename_to_delete == current_file_name or aria2_file == file:
                    ...
                    if os.path.isfile(path_file):
                        try:
                            ...
                            print(
                                f"Model moved to trash based on filename: {path_file}"
                            )
                        except:
                            os.remove(path_file)
            ...
            for file_entry in version["files"]:
                if file_entry["hashes"].get("SHA256") == sha256:
                    for image in version["images"]:
                        if image["type"] == "image":
                            url_with_width = re.sub(
                                r"/width=\d+", f"/width={image['width']}", image["url"]
                            )

                            response = requests.get(
                                url_with_width, proxies=proxies, verify=ssl
                            )
                            if response.status_code == 200:
                                ...
                                print(
                                    f"Failed to save preview. Status code: {response.status_code}"
                                )
    ...
    if image_location:
        if sub_image_location:
            ...
            image_path = os.path.join(
                _api.contenttype_folder(
                    content_type, desc, custom_folder=image_location
                )
            )
            ...
            if (
                ...
                image_path = os.path.join(
                    image_path, sub_folder.lstrip("/").lstrip("\\")
                )
        else:
...
def save_images(
    preview_html, model_filename, install_path, sub_folder, api_response=None
):
...
def card_update(gr_components, model_name, list_versions, is_install):
    ...
    if is_install and not gl.download_fail and not gl.cancel_status:
        ...
        version_choices_clean = [
            version
            if version + " [Installed]" != version_value_clean
            else version_value_clean
        ...
        version_choices_clean = [
            version
            if version.replace(" [Installed]", "") != version_value_clean
            else version_value_clean
            for version in version_choices
        ]

    first_version_installed = "[Installed]" in version_choices_clean[0]
    any_later_version_installed = any(
        "[Installed]" in version for version in version_choices_clean[1:]
    )
...
def model_from_sent(model_name, content_type):
    ...
    for content_type_item in content_type:
        ...
        for folder_path, _, files in os.walk(folder, followlinks=True):
            for file in files:
                if (
                    (not model_file_length or len(file) < model_file_length)
                    and file.startswith(model_name)
        ...
        if os.path.exists(html_file):
            with open(html_file, "r", encoding="utf-8") as html:
                ...
                if local_path_in_html:
                    output_html = convert_local_images(
                        output_html, os.path.dirname(html_file)
                    )
    ...
    if not output_html:
        ...
        if not modelID_failed:
            json_data = _api.request_civit_api(
                f"https://civitai.com/api/v1/models?ids={modelID}&nsfw=true"
            )
        else:
            json_data = None
        ...
        if not isinstance(json_data, dict):
            ...
            output_html = _api.update_model_info(
                None, model_versions.get("value"), True, modelID, json_data, True
            )
...
def send_to_browser(model_name, content_type, click_first_item):
    ...
    if not output_html:
        ...
        if not modelID_failed:
            gl.json_data = _api.request_civit_api(
                f"https://civitai.com/api/v1/models?ids={modelID}&nsfw=true"
            )
...
def convertCustomFolder(
    folderValue, basemodel, nsfw, author, modelName, modelId, versionName, versionId
):
    replacements = {
    ...
    converted_folder = os.sep.join(
        part for part in converted_folder.split(os.sep) if part
    )
...
def getSubfolders(
    ...
    try:
        ...
        for key, value in config_data.items():
            if basemodel:
                try:
                    converted_value = convertCustomFolder(
                        value,
                        basemodel,
                        nsfw,
                        author,
...
def clean_description(desc):
    try:
        ...
        print(
            "Python module 'BeautifulSoup' was not imported correctly, cannot clean description. Please try to restart or install it manually."
        )
        cleaned_text = desc
    return cleaned_text
...
def save_model_info(
    ...
    save_path, filename = get_save_path_and_name(
        install_path, file_name, api_response, sub_folder
    )
    image_path = get_image_path(install_path, api_response, sub_folder)
    ...
    result = find_and_save(
        api_response, sha256, file_name, json_file, False, overwrite_toggle
    )
    if result != "found":
        result = find_and_save(
            api_response, sha256, file_name, json_file, True, overwrite_toggle
        )

    if preview_html:
        if use_local:
            img_urls = re.findall(
                r'data-sampleimg="true" src=[\'"]?([^\'" >]+)', preview_html
            )
            for i, img_url in enumerate(img_urls):
                img_name = f"{filename}_{i}.jpg"
                preview_html = preview_html.replace(
                    img_url, f"{os.path.join(image_path, img_name)}"
                )
...
def get_models(file_path, gen_hash=None):
    ...
    try:
        if not modelId or not modelVersionId:
            response = requests.get(
                by_hash, timeout=(60, 30), proxies=proxies, verify=ssl
            )
            if response.status_code == 200:
...
def file_scan(
    ...
    if not all_model_ids:
        ...
        print(
            'Could not retrieve any Model IDs, please make sure to turn on the "One-Time Hash Generation for externally downloaded models." option if you haven\'t already.'
        )
        no_update = True
        gl.scan_files = False
        time.sleep(2)
            ...
            while url:
                try:
                    if progress is not None:
                        progress(
                            url_done / url_count,
                            desc=f"Sending API request... {url_done}/{url_count}",
                        )
                    response = requests.get(
                        url, timeout=(60, 30), proxies=proxies, verify=ssl
                    )
                    if response.status_code == 200:
                        ...
                        print(
                            f"Error: Received status code: {response.status_code} with URL: {url}"
                        )
                        print(response.text)
                        return (
    ...
    if from_ver:
        ...
        outdated_set = {
            model
            for model in outdated_set
            if model[0] not in {updated_model[0] for updated_model in updated_set}
        }
        ...
        if len(all_model_ids) == 0:
            ...
            return (
                gr.HTML.update(
                    value='<div style="font-size: 24px; text-align: center; margin: 50px !important;">No updates found for selected models.</div>'
                ),
                gr.Textbox.update(value=number),
            )
    ...
    base_url = "https://civitai.com/api/v1/models?limit=100&nsfw=true"
    gl.url_list = {
        i + 1: f"{base_url}{''.join(chunk)}" for i, chunk in enumerate(model_chunks)
    }
    ...
    if from_ver:
        ...
        return (
            gr.HTML.update(
                value='<div style="font-size: 24px; text-align: center; margin: 50px !important;">Outdated models have been found.<br>Please press the button above to load the models into the browser tab</div>'
            ),
            gr.Textbox.update(value=number),
        )
        ...
        return (
            gr.HTML.update(
                value='<div style="font-size: 24px; text-align: center; margin: 50px !important;">Installed models have been loaded.<br>Please press the button above to load the models into the browser tab</div>'
            ),
            gr.Textbox.update(value=number),
        )
        ...
        for file_path, id_value in zip(file_paths, all_ids):
            install_path, file_name = os.path.split(file_path)
            save_path, name = get_save_path_and_name(
                install_path, file_name, api_response
            )
            ...
            if (
                ...
                or create_html
                and overwrite_toggle
            ):
                preview_html = _api.update_model_info(
                    None,
                    model_versions.get("value"),
                    True,
                    id_value,
                    api_response,
                    True,
